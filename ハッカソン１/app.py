...
grammarNote（文法構造の解説）に加え、nuance（ネイティブの使用ニュアンス解説）も
モックとして返す。実翻訳・実解析ロジックへの差し替えは後続タスクとする。
...
def build_mock_reply(text):
    ...
    詳細設計書 4.2 のレスポンス構造
        reply: { en, ja, grammarNote, nuance, phraseMap }
    に完全準拠したダミーデータを返す。

    nuance は「ネイティブが実際にどんなニュアンスで使っているか」を補足する解説文。
    grammarNote（文法構造の説明）とは役割を分け、発表原稿で説明している
    「ニュアンス解説」に対応させるため追加。

    実際の翻訳・文法解析・phraseMap抽出・ニュアンス解析ロジックは未実装
    （現段階は固定文のモックであり、次フェーズで実API化）。
    """
    return {
        "en": text,
        "ja": "（モック応答）それについてもっと教えていただけますか？",
        "grammarNote": "「Could you ~?」は依頼を丁寧に行う定型表現です。",
        "nuance": "命令口調を避けたいネイティブが日常的によく使う言い方で、"
                  "友人相手からビジネスシーンまで、カジュアルすぎず堅すぎない"
                  "ちょうどよい丁寧さとして好まれています。",
        "phraseMap": {
            "Could you": {
                "id": "p001",
